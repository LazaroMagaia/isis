...
export default function IndexCategories() {
    ...
    const handleFilter = (e) => {
        e.preventDefault();
        router.get(route('admin.medicinecategories.index'), { search }, { preserveState: true, replace: true });
    };
    ...
    const handleDelete = (id, name) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                router.delete(route('admin.medicinecategories.destroy', id), {
                    preserveState: true,
                    onSuccess: () => {
                        Swal.fire('Excluído!', `Categoria ${name} foi excluída.`, 'success');
                    },
                    onError: () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        if (modalMode === 'create') {
            router.post(route('admin.medicinecategories.store'), formData, {
                preserveState: true,
                onSuccess: () => {
                    setModalOpen(false);
            ...
            router.put(route('admin.medicinecategories.update', formData.id), formData, {
                preserveState: true,
                onSuccess: () => {
                    setModalOpen(false);
    ...
    return (
        <DashboardLayout title="Categorias de Medicamentos">
