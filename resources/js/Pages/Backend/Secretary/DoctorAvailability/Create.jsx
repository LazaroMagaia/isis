...
export default function CriarDisponibilidade({ doctor_id, availability }) {
    const form = useForm({
        doctor_id,
        datas: availability?.datas || [],
        duracao_slot: availability?.duracao_slot || 30,
        ativo: true,
    });
    ...
    const enviar = (e) => {
        ...
        form.post(route('secretary.doctor-availability.store'), {
            data: payload,
            onSuccess: () => Swal.fire('Sucesso', 'Disponibilidade criada com sucesso!', 'success'),
            onError: (errors) => Swal.fire('Erro', Object.values(errors)[0], 'error'),
        });
    };

    return (
        <DashboardLayout title="Criar Disponibilidade">
            <div className="w-full min-w-[320px] max-w-7xl mx-auto py-10 flex flex-col gap-6 px-4 sm:px-6 lg:px-8">
                <Link
                    href={route('secretary.doctor-availability.show', { id: doctor_id })}
